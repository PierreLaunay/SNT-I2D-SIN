...
sml={(s,m):[i for ((i,s1),m1) in zip(enumerate(sl),ml) if s==s1 and m==m1] for ((i,s),m) in zip(enumerate(sl),ml)}
centre = sml[min(sml)]
rayon=min(sml)[1]
diametre=max(sml)[1]
list_centre = [graph_data.get("nodes")[c] if c in centre else "" for c in range(len(sl))]
list_rayon = [rayon if c in centre else "" for c in range(len(sl))]
list_diametre = [diametre if (s,m)==max(sml) else "" for ((i,s),m) in zip(enumerate(sl),ml)]
