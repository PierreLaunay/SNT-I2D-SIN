...
class IHM(tkinter.Frame) : # class IHM(Frame)
    def __init__(self, fenetre, Noms,table,boucle, bgc=None):
        ...
        self.bgc = bgc if bgc else "#ffffff"
        ...
        for i in range(self.numberLines-1): # remplissage table
            ...
            for j in range(self.numberColumns-1):
                ...
                cell = tkinter.Entry(self)
                if hasattr(self, "rayon") and hasattr(self, "diametre") and valeur in [self.rayon, self.diametre]:
                    cell.configure(width=self.nb, background = self.bgc)
                else:
                    cell.configure(width=self.nb)
